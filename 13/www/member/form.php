<script>
    function check_input() {
        if (!documen.member.id.value) {
            alert("아이디를 입력하세요!");
            document.member.id.focus();
            return;
        }
        if (!documen.member.pass.value) {
            alert("비밀번호를 입력하세요!");
            document.member.pass.focus();
            return;
        }
        if (!documen.member.pass_confirm.value) {
            alert("비밀번호 확인를 입력하세요!");
            document.member.pass_confirm.focus();
            return;
        }
        if (!documen.member.name.value) {
            alert("이름을 입력하세요!");
            document.member.name.focus();
            return;
        }
        if (!documen.member.email.value) {
            alert("이메일 주소를 입력하세요!");
            document.member.email.focus();
            return;
        }
        if (!documen.member.pass.value != !documen.member.pass_confirm.value) {
    ...
    function check_id() {
        window.open("check_id.php?id=" + documemt.member.id.value, "IDcheck", "left=700, top=300, width=380, height=160, scrollbars=no, resizeable=yes");
    }
</script>
